...
6. Verify the result. `set-cell`, `swap-cell-text`, `set-cell-shading`, `set-table-borders`, `set-cell-borders` and `normalize-table-borders` support result reuse when `ok`, `applied`, `verified` and `inspection_complete` are true, document/target match, error/failure arrays are empty and the fingerprint is non-null. Cell text/shading commands also require `readback.matches_requested:true`. Reuse that fingerprint for the next operation without an extra full query; the next mutation still recomputes the live full fingerprint. Reinspect for additional scope or visual evidence, and never reuse a failed or incomplete result. After `create-table`, identify the new table before another indexed operation: `table_count` alone is not its index.
